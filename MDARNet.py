...
class LRDN(nn.Module):
    def __init__(self,in_channel,out_channel,num_dab=4,num_rrg=4,n_feats=64,reduction=16):
        ...
        modules_body = [ARG(n_feat=n_feats, reduction=reduction, num_dab=num_dab)
                         for _ in range(self.num_rrg)]
        self.conv3 = nn.Sequential(
            nn.Conv2d(in_channels=n_feats*(self.num_rrg+1),out_channels=n_feats,kernel_size=1,padding=0,stride=1),
            SepConv(in_channel=n_feats, out_channel=n_feats, kernel_size=kernel_size)
        )
...
class MDARNet(nn.Module):
    def __init__(self, H=0, W=0, batch_size=0):
        ...
        self.LRDN1 = LRDN(in_channel=12,out_channel=12,num_dab=4,num_rrg=4,n_feats=64,reduction=16)
        self.LRDN2 = LRDN(in_channel=12,out_channel=12,num_dab=4,num_rrg=4,n_feats=64,reduction=16)
    ...
    def forward(self, x):
        ...
        subband1[:,0:3,:,:] = x
        t3 = time.time()
        x = self.LRDN1(subband1)
        t4 = time.time()
        lo1 = x
        x = MWT_sys(x, ft_dims=self.ft_dims, paths=self.paths,
                    row_sub=self.row_cub, col_sub=self.col_cub, row=self.row, col=self.col,
                    filters=self.filters1)  # Nx3xWxH
        sum_time = t2-t1+t4-t3
        return lo1,lo2,x,sum_time
...
if __name__ == '__main__':
    ...
    # net = SCDAB(64)
    net = MDARNet(H=128,W=128,batch_size=1).cuda()
    print_network(net)
    out = net(x)
