...
for idx_set = 1:num_set
    file_path = strcat('./albation/', datasets{idx_set}, '/Rain100H/derain/');
    gt_path = strcat('./data/','ours/Rain100H/', '/target/');
    ...
    if img_num > 0 
        for j = 1:img_num 
           ...
           %fprintf('For %s image PSNR: %f SSIM: %f\n', image_name, psnr_val, ssim_val);
