...
def get_args():
    ...
    parser.add_argument("--batchSize", type=int,required=False, default=4,help="Training batch size")
    parser.add_argument("--pachSize", type=int,required=False, default=128,help="Training batch size")
    parser.add_argument("--epochs", type=int, required=False , default=250, help="Number of training epochs")
...
if __name__ == '__main__':
    ...
    # loading datasets	
    #train_dataset = Dataset(data_path=opt.train_data)
    #train_loader = DataLoader(dataset=train_dataset, num_workers=4, batch_size=opt.batchSize, shuffle=True,pin_memory=True)
    ...
    model = Net_test1(H=H,W=W,batch_size=opt.batchSize)
    #model = NET(in_channel=3,out_channel=3)
